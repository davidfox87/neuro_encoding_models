...
def plot_model_output(orn_responses, g1_, g2_, iapp, dur):
	...
	for i in range(orn_responses.shape[1]):
		# fig, ax = plt.subplots(5, 1, sharex=True)
		vm[:, i], model_pn_responses[:, i], istm, hcurr, sps, g1[:, i], g2[:, i], i_syn[:, i], inh = synapse.run(
		 	orn_responses[:, i], g1_, g2_, iapp=iapp, dur=dur)
...
if __name__ == "__main__":
	...
	control_pn_responses = plot_model_output(orn_responses, control_g1, g2, iapp=iapp_control, dur=30.)

	unc13_g1 = 0
	unc13_pn_responses = plot_model_output(orn_responses, unc13_g1, g2, iapp=iapp_unc13, dur=30.)

	mean_orn_responses = np.ones((7, 1))
	mean_pn_responses = np.ones((7, 1))
	mean_pn_responses_unc13 = np.ones((7, 1))
	for i in range(7):
		mean_orn_responses[i] = np.mean(orn_responses[int(6 * fs): int(7 * fs), i])
		mean_pn_responses[i] = np.mean(control_pn_responses[int(6 * fs): int(7 * fs), i])
		mean_pn_responses_unc13[i] = np.mean(unc13_pn_responses[int(6 * fs): int(7 * fs), i])
	...
	control_pn_responses = plot_model_output(orn_responses, control_g1, g2, iapp=iapp_control, dur=35.)

	orn_responses = read_orn_fr('../datasets/neural/meanORNFr.mat', 'chirps')
	unc13_pn_responses = plot_model_output(orn_responses, unc13_g1, g2, iapp=iapp_unc13, dur=35.)
	...
	f2_ax9.plot(freq, amp_pn_control / amp_orn, 'k')
	f2_ax9.plot(freq, amp_pn_unc13 / amp_orn, 'm')
	#plt.xscale('log')
	f2_ax9.set_xlabel('Frequency (Hz)')
	f2_ax9.set_ylabel('PN amp/ORN amp')
	f2_ax9.set_title('Amplitude Filtering')

	f2_ax10.scatter(freq, (latency_pn_control - latency_orn) * (2 * np.pi * freq), 150, 'k')
	f2_ax10.scatter(freq, (latency_pn_unc13 - latency_orn) * (2 * np.pi * freq), 150, 'm')
